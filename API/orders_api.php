...
{
// 	 $query = "SELECT * FROM tbl_orders as o left join tbl_users as user on user.id=o.user_id left join tbl_address as addr on addr.addr_id=o.addr_id left join tbl_products as p on p.p_id=o.prod_id WHERE o.status = 1 order by o.booking_id  DESC";
    $query = "SELECT * FROM tbl_orders as o left join tbl_users as user on user.id=o.user_id left join tbl_address as addr on addr.addr_id=o.addr_id left join tbl_country as c on c.country_id=addr.country_id left join tbl_state as s on s.state_id=addr.state_id left join tbl_city as city on city.city_id=addr.city_id left join tbl_area as area on area.area_id=addr.area_id left join tbl_products as p on p.p_id=o.prod_id WHERE o.status = 1 order by o.booking_id  DESC";
